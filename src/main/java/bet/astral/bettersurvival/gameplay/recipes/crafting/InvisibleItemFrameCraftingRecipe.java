...
public class InvisibleItemFrameCraftingRecipe implements Recipe, Listener {
	...
	public InvisibleItemFrameCraftingRecipe() {
		this.result = new ItemStack(Material.ITEM_FRAME);
		this.key = BetterSurvival.instance().key("invisible_item_frame");
		...
		this.result.editMeta(meta->{
			...
			meta.displayName(Component.text("Invisible Item Frame").color(ItemRarity.UNCOMMON.color()).decoration(TextDecoration.ITALIC, false));
